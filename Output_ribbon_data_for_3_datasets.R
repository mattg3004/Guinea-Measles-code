ribbon.plot.NZoo.3.sims <- function(Sims.A, Cases.A, End.Week.A,
  ...
  quantile.05 <- function(y) {return(quantile(y, prob=.05))}
  quantile.95 <- function(y) {return(quantile(y, prob=.95))}
  ...
  gg.curve <- ggplot(A, aes(x=Week, y=Cases,id = iter, colour=as.factor(run), fill=as.factor(run))) + 
    ...
    theme(legend.position="bottom", legend.title=element_blank()) #+ labs(title = "N'Zoo")
  gg.curve
  return(gg.curve)
}
...
ribbon.plot.Non.NZoo.3.sims <- function(Sims.A, Cases.A, End.Week.A,
  ...
  gg.curve <- ggplot(A, aes(x=Week, y=Cases,id = iter, colour=as.factor(run), fill=as.factor(run))) + 
    ...
    stat_summary(geom="ribbon", fun.ymin = quantile.05, fun.ymax= quantile.95, alpha=0.25, colour=NA) +
    geom_vline(xintercept = 17, color = rgb(0,0.5,1), linetype = "dashed") + 
    geom_vline(xintercept = 13, color = rgb(1,0.3,0), linetype = "dashed")  + xlim(c(Week1,27)) +
    theme(legend.position="bottom", legend.title=element_blank())  + labs(title = S.prefect)
...
ribbon.plot.NZoo.2.sims.method.comparison <- function(Sims.A, Cases.A, End.Week.A,
                                    Sims.B, Cases.B, End.Week.B,
                                    Week1){
  
  A = matrix(0, 1000 * nrow(Cases.A) + 1000 * nrow(Cases.B) 
             + 1000 * dim(Sims.A)[2] + 1000 * dim(Sims.B)[2], 4)
  count = 1
  for(i in 1 : 1000){
    for(j in 1 : nrow(Cases.A)){
      A[count, 1] = 1
  ...
  A = data.frame(A)
  colnames(A) = c("run", "Week", "Cases", "iter")
  A$run = factor(A$run, levels=c(1, 2), labels=c("Fixed seasonality amplitude", "Variable seasonality amplitude"))
  gg.curve <- ggplot(A, aes(x=Week, y=Cases,id = iter, colour=as.factor(run), fill=as.factor(run))) + 
    stat_summary(fun.y=mean, geom="point") + stat_summary(fun.y=mean, geom="line") + 
    stat_summary(geom="ribbon", fun.ymin = quantile.05, fun.ymax= quantile.95, alpha=0.25, colour=NA) +
    geom_vline(xintercept = 17, color = rgb(0,0.5,1), linetype = "dashed") + 
...
ribbon.plot.Non.NZoo.2.sims.method.comparison <- function(Sims.A, Cases.A, End.Week.A,
                                        Sims.B, Cases.B, End.Week.B,
                                        Week1, S.prefect){
  
  A = matrix(0, dim(Cases.A)[1]*dim(Cases.A)[2] + dim(Cases.B)[1] * dim(Cases.B)[2] +
               1000 * dim(Sims.A)[2] + 1000 * dim(Sims.B)[2], 4)
  count = 1
  for(i in 1 : 1000){
    for(j in 1 : dim(Sims.A)[2] ){
